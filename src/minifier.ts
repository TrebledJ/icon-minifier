...
import { createParser, AstSelector } from 'css-selector-parser';
...
//  - directory
//  - options
//      - files to crawl (default: html)
//      - relative CSS out directory (default: ./css)
//      - relative webfont out directory (default: ./webfonts)
//      - cache online files (default: yes)
//      - replace <link> with new CSS file. (default: no)
//      - autoreplace with SVGs (v2)
// 
...
// 
// Principle
//  - Code should not rely (too much) on hardcoding behaviour specific to a webfont (e.g. FontAwesome variants).
// 
// The Data
//  - Icons are expressed as [variant, icon-class].
//    - For example, in Font Awesome, we may have [fa-brands, fa-google].
// 
...
// 
// 7. Construct a CSS based on the font file.
//    Also, include any modifier classes found.
// 
// 8. Replace CSS <link> in HTML files.
// 


const WEBFONTS = {
    'font-awesome': {
        filePattern: 'font-?awesome',
        // prefix: 'fa-',
    },
};
...
class Icon {
    ...
    constructor(name: string,/*  variant?: string, */ modifiers: string[] = []) {
        this.name = name;
...
class IconMinifier {
    ...
    async minify() {
        ...
        for (const cssFile of cssFiles) {
            ...
            // const variantClassCheckers = cssParser.getVariantClassCheckers();
            // for ()
            ...
            // 3.
            // const fontFiles = cssParser.getFontFiles();
            // if (!fontFiles) {
            //     throw new Error(`No font file urls found in css file ${cssFile}. False positive for font file?`);
            // }
            // for (const fontFile of fontFiles) {
            //     const fontPath = CSSParser.getRelativeFile(cssFile, fontFile);
            //     fontManager.addFont(fontPath);
            //     // TODO: get codepoints for each font?
            // }
